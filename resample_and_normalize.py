...
from cropping import get_case_identifier_from_npz, ImageCropper
...
class GenericPreprocessor(object):
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
        ...
        for i in range(num_stages):
            all_args = []
            output_folder_stage = os.path.join(output_folder, data_identifier + "resampled%d" % i)
...
if __name__ == "__main__":
    '''
    python resample_and_normalize.py --spacing_zxy 1. 0.78125 0.78125 --data_dir /mnt/hdd/sda/ygeo/code/kits23/kits23_project/dataset_converted/nnUNet_cropped_data/Task100_KiTS23/ --output_dir /mnt/hdd/sda/ygeo/tmp_kits23/ --plans_file plans.pkl --num_threads 4
    '''
   
    
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--spacing_zxy",  nargs="+", help="List of spacings in [z,x,y] directions", type = float)
    ...
    parser.add_argument("--output_dir", type = str, default = "/mnt/hdd/sda/ygeo/tmp_kits23/", help="output directory")
    parser.add_argument("--plans_file", type = str, default = "config.pkl", help="plans pkl file path")
    ...
    with open(args.data_dir + args.plans_file, 'rb') as f:
        plans = pickle.load(f)
    ...
    target_spacings = [list(args.spacing_zxy) if args.spacing_zxy else plans['plans_per_stage'][1]['current_spacing']]
    print(target_spacings)
    
    
    output_dir = args.output_dir 
    data_dir = args.data_dir
    preprocessor.run(target_spacings, data_dir, output_dir, "", num_threads)
    ...
    filenames = glob.glob(output_dir + 'resampled0/*.npz')
    i = 1
    for f in filenames:
        img = np.load(f)['data']
        ...
        if i > w*h:
            break
    plt.savefig("sample_images.png", bbox_inches='tight', pad_inches=0)
